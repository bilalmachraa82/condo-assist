...
const APP_URL = Deno.env.get("APP_BASE_URL") ??
  "https://condo-assist.lovable.app";
  ...
  try {
    ...
    const { data: settings } = await supabase
      ...
      .in("key", [
        ...
        "insurance_alerts_recipients",
        "insurance_overdue_repeat_days",
      ]);
    ...
    const recipients: string[] =
      Array.isArray(map.get("insurance_alerts_recipients"))
    ...
    for (const r of (rows ?? []) as InsuranceStatusRow[]) {
      ...
      const { data: existing } = await supabase
        ...
        .eq("insurance_id", r.insurance_id)
      ...
      const { error: claimError } = await supabase.from("insurance_alerts_log")
        .insert({
    ...
    const subject = `[Luvimg] ${newAlerts.length} ${
      newAlerts.length === 1
        ? "seguro requer atenção"
        : "seguros requerem atenção"
    }`;
    ...
    for (const bid of buildingsAlerted) {
      ...
      await supabase.from("notifications").insert({
        ...
        priority: items.some((i) => i.status === "overdue")
          ? "critical"
          : "normal",
...
function buildDigestHtml(alerts: InsuranceStatusRow[]): string {
  const buckets: Record<string, InsuranceStatusRow[]> = {
    overdue: [],
    due_soon_30: [],
  };
  ...
  const section = (
    ...
    const rows = list.map((r) => `
      ...
      escape(r.building_code)
    } - ${escape(r.building_name)}</div>
          ${
      r.fractions_included
        ? `<div style="font-size:11px;color:#94a3b8;margin-top:2px;">Fracções: ${
          escape(r.fractions_included)
        }</div>`
        : ""
    }
        </td>
      ...
      escape(coverageLabel(r.coverage_type))
    }</div>
          ${
      r.broker
        ? `<div style="font-size:11px;color:#94a3b8;">Mediador: ${
          escape(r.broker)
        }</div>`
        : ""
    }
  ...
  return `<!doctype html><html><body style="font-family:Arial,sans-serif;background:#f4f5f7;padding:20px;color:#222;">
    <div style="max-width:760px;margin:0 auto;background:#fff;border-radius:10px;padding:24px;box-shadow:0 2px 6px rgba(0,0,0,0.06);">
      ...
      <p style="margin:0 0 16px;color:#64748b;font-size:13px;">Resumo automático gerado em ${
    formatDate(new Date().toISOString().slice(0, 10))
  }. Use os dados abaixo para validar e accionar a renovação.</p>
